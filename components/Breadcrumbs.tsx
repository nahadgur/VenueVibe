        ...
        {items.map((item, index) => {
          ...
          return (
            <li key={item.href} className="flex items-center gap-1.5">
              ...
              {isLast ? (
                <span className="text-[13px] font-light text-[#2C2418]">{item.label}</span>
              ) : (
                <Link href={item.href} className="text-[13px] font-light text-[#A69580] hover:text-[#D4654A] transition-colors">{item.label}</Link>
              )}
            </li>
          );
        })}
